...
pophu_raster <- spTransform(pophu, crs(dem))
pophu_raster$area <- gArea(pophu_raster, byid = TRUE)
for(i in 1:length(pophu_raster$area)){
  pophu_raster$PD[i] <- pophu_raster$POP10[i] / pophu_raster$area[i]
}
pophu<-pophu_raster
pophu_df <- fortify(pophu)
